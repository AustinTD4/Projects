...
if __name__ == '__main__': 
    ...
    filename = 'generalist_starter_culture'

    if not os.path.exists(f'{stat_directory}_{filename}'):
        os.makedirs(f'{stat_directory}_{filename}')

    # Starter cultures from specialized agents trained against just one boss
    populations = {}
    ...
    # Gather starter culture from specialists
    new_pop = []
    for i in range(4,8):
        new_pop.append(pop.iloc[(i*100)+80:(i*100)+100])
    ...
    np.savetxt(f"{stat_directory}_{filename}/Starter_Culture_Backup_[5-8].csv", starter_pop1, delimiter=",")
    ...
    np.savetxt(f"{stat_directory}_{filename}/Starter_Culture_Backup_[1-4].csv", starter_pop2, delimiter=",")
    ...
    np.savetxt(f"{stat_directory}_{filename}/Starter_Culture_NEW.csv", full_starter, delimiter=",")
    ...
    filename = f'GENERALIST_FULL_STARTER_TEST_PSO'
    print(filename)

    if not os.path.exists(f'{stat_directory}_{filename}'):
        os.makedirs(f'{stat_directory}_{filename}')

    # Evolve the starter culture with particle swarm optimization
    evoman_train_set(filename, pop, runs, save=True, set=[1,2,3,4,5,6,7,8], pop_size=1000, generalist=True, particle_swarm_optimization=True, multiple_mode=True, generations=50)
    ...
    filename = f'GENERALIST_FULL_STARTER_TEST_DE'
    print(filename)

    if not os.path.exists(f'{stat_directory}_{filename}'):
        os.makedirs(f'{stat_directory}_{filename}')

    # Evolve the starter culture with differential evolution
    evoman_train_set(filename, pop, runs, save=True, set=[1,2,3,4,5,6,7,8], pop_size=1000, generalist=True, particle_swarm_optimization=False, differential_evolution=True, multiple_mode=True, generations=50)
